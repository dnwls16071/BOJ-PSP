...
for idx, val in enumerate(lst):
    ...
    result.append(((idx+1) * val, val))

result.sort(reverse=True)
print(result[0][1], result[0][0])

# 1246(온라인 판매) 문제는 구글링과 끊임없는 고민을 거듭했다. 
# enumerate()를 사용하여 인덱스와 값을 받아오고 (인덱스 번호 + 1)의 값이 전체 인원 수를 초과하면 break를 걸어주어 일종의 예외처리 개념을 사용하였고 그 다음 내림차순으로 정렬하여 각각의 값으로 산정했을때 얻을 수 있는 가장 큰 수익을 출력했다.
