...
pocketmon_dict = {}     # 딕셔너리 자료형 선언
for i in range(1, N+1):
    pocketmon = input().rstrip()
    pocketmon_dict[i] = pocketmon   # pocketmon_dict의 i번째에 포켓몬 이름을 넣는다.
    pocketmon_dict[pocketmon] = i   # pocketmon_dict의 이름에 해당하는 번호를 넣는다.

for i in range(M):
    quiz = input().rstrip() # rstrip의 인자가 없을 경우 오른쪽 공백 제거
    if quiz.isdigit():  # 숫자로만 구성되어있는지 확인
        print(pocketmon_dict[int(quiz)])
    else:
        print(pocketmon_dict[quiz])

# 문제를 보고 딕셔너리를 이용해야겠다는 생각이 들어 바로 코드를 작성했다.
# 근데 문제를 풀면서 궁금증이 생겼다.
# 문제에서 rstrip()을 사용하지않으면 런타임에러(Keyerror)를 출력하면서 WA가 나온다.

# sys 모듈
# import sys
# input = sys.stdin.readline
# 위 모듈은 input보다 입력을 빠르게 받아들이지만 공백문자도 같이 입력을 받게 된다.
# 따라서 문자의 공백을 지워주는 rstrip()을 써줘야된다.