...
def recursive_function(i, N):
    print("____" * i + '"재귀함수가 뭔가요?"')
    # 종료조건을 명시해줘야한다.(재귀 최대 깊이 초과 방지)
    if i == N:
...
# 이 문제는 재귀함수를 다룬 문제이다.
# 재귀함수는 while문과 비슷한 개념이라고 생각하면되는데 이 때 재귀함수가 무한루프를 돌게 되면 최대 재귀 깊이를 초과하게되어 프로그램이 종료될 수 있다.
