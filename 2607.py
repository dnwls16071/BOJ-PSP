...
for _ in range(N-1):
    ...
    for i in range(len(input_word)):
        t = input_word.pop(0)
        if t in compare_word:
            compare_word.remove(t)
        else:
            compare_word.append(t)
    if (len(compare_word) == 0 and len(input_word) == 0) or (len(compare_word) == 1 and len(input_word) == 0) or (len(compare_word) == 1 and len(input_word)==0) or (len(compare_word) == 0 and len(input_word) == 1):
        cnt += 1
    print(cnt)
