...
# 1. 최고의 피자란 피자 가게에서 주문할 수 있는 피자 중 1원당 열량기 가장 높은 피자
# 2. 토핑 N개에서 여러 종류 선택 가능, 전혀 선택하지 않을 수도 있다
# 3. 피자의 가격 = 도우의 가격 + 토핑의 가격 * 토핑 종류 개수
# 4. 피자의 열량 = 도우의 열량 + 토핑의 열량

# 참고한 코드
N = int(input())    # 토핑의 종류의 수 N
A, B = map(int, input().split())    # 도우의 가격 A와 토핑의 가격 B
C = int(input())    # 도우의 열량 C
D = []  # 각 토핑의 열량을 저장할 변수 D
for i in range(N):
    D.append(int(input()))
D.sort(reverse=True)

answer = C / A  # 도우의 열량을 도우의 가격으로 나눈다.
